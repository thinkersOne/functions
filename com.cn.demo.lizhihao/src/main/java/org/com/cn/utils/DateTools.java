...
public class DateTools  {
    ...
    public static String getDate17(){
        ...
        Date now = new Date();
        String nowString = sdf.format(now);
        return nowString+"000000000";//HHmmssSSS
    }
    /***
     * 获取时间的截止时间 格式：yyyy-MM-dd 23:59:59
     * @return
     */
    public static Long getDate18(Long date){
        ...
        Long result = zero.getTime() + 86399000L;
        return result;
    }
    ...
    /***
     * 时间字符串转换 ： 传入：2010-01-01 12:00:00 转出：20100101120000
     * 			      传入：2010-01-01 转出：20100101
     * 			      传入：2010-01-01 12:00:00:000 转出：20100101120000000
     * @param date
     * @return
     */
    public static String parseDateToTime(String date) {
        ...
        SimpleDateFormat smf = new SimpleDateFormat(str+" HH:mm:ss");
        SimpleDateFormat smf1 = new SimpleDateFormat("yyyyMMddHHmmss");
        if (date.indexOf("年")!=-1) {
            str="yyyy年MM月dd日";
            smf = new SimpleDateFormat(str);
            smf1 = new SimpleDateFormat("yyyyMMdd");
        }
        if (date.trim().length()==10) {
            smf = new SimpleDateFormat(str);
            smf1 = new SimpleDateFormat("yyyyMMdd");
        }
        if (date.trim().length()==23) {
            smf = new SimpleDateFormat(str+" HH:mm:ss:SSS");
            smf1 = new SimpleDateFormat("yyyyMMddHHmmssSSS");
        }
        Date date1 = new Date();
        try {
            date1 = smf.parse(date);
    ...
    /***
     * 时间字符串转换 ： 传入：20100101120000 转出：2010-01-01 12:00:00
     * 			     传入：20100101 转出：2010-01-01
     * 			     传入：20100101120000000转出：2010-01-01 12:00:00:000
     * @param date
     * @return
     */
    public static String parseTimeToDateString(String date) {
        if (StringUtils.isEmpty(date)) {
            return date;
        }
        if (date.length()<6) {
        ...
        if (date.trim().length()==6) {
            smf1 = new SimpleDateFormat("yyyy年MM月");
            smf = new SimpleDateFormat("yyyyMM");
        }
        if (date.trim().length()==8) {
            smf1 = new SimpleDateFormat("yyyy-MM-dd");
            smf = new SimpleDateFormat("yyyyMMdd");
        }
        if (date.trim().length()==17) {
            smf1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS");
            smf = new SimpleDateFormat("yyyyMMddHHmmssSSS");
        }
        Date date1 = new Date();
        try {
            date1 =smf.parse(date);
    ...
    public static String getDateStr(String str) {
        ...
        if (str.length() == 8) {
            ...
            return str.substring(0, 4) +'-'+ str.substring(4, 6) +'-'+ str.substring(6, 8)
                    + " " + str.substring(8, 10) + ":" + str.substring(10, 12)
                    + ":" + str.substring(12, 14) + "."+ str.substring(14, 17);
        }else{
            return str;
        }
    }
    ...
    //将Timestamp类型转化为14位的字符串类型
    public static String changeTimestampToString14(Timestamp expireDate){
        if(expireDate == null) return "";
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date1;
        String termEndTime = null;
        try{
            date1 = new Date(expireDate.getTime());
            termEndTime = sdf.format(date1);
        }catch(Exception e){
            log.error("供应商传入的服务到期时间格式不对，请重新填写！");
        }
        return termEndTime;
    }
    //将Timestamp类型转化为14位的字符串类型
    public static String changeStringToString14(String expireDate){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddhhmmss");
        Date date1;
        String termEndTime = null;
        try{
